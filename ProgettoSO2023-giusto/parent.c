...
#define STOP "stop"
//shared memory dove ci saranno variabili importanti per il controllore
struct sharedMemory
{
     pid_t pidParent;
     int n_pro;
...
// handler per gestire il segnale dal controllore
...
void filosofoStallo(int i, int sizeShm)
{
     ...
     while (1)
     {
          ...
          printf("%d prende forchetta %d - destra\n", i, i);
          while(sem_trywait(&forchette[j]) != 0 && sem_trywait(&forchette[j]) != 0){
                shmStato_ptr[i] = AFFAMATO;
          }
          //shmStato_ptr[i] = AFFAMATO;
...
int main(int argc, char *argv[])
{
     ...
     for (int i = 0; i < n_pro; i++)
     {
          ...
          else if (pid == 0)
          {
               ...
               if(ab_stallo == 1){
                    ...
                    filosofoStallo(i, sizeof(shm)); //filosofo senza stallo
               }
          }
     }

     signal(SIGINT, SigIntHandler); //signal handler del parent
     //if(riv_star == 1){ //da rivedere
     signal(SIGUSR1, SigUsr1Handler);
     //}
     //execl("./controllore > filelog.txt", "./controllore > filelog.txt", NULL);
     printf("parent: pid %d\n", getpid());
     ...
     for (int i = 0; i < n_pro; i++)
     {
          ...
          kill(shm_ptr->pidControllore, SIGINT);         
